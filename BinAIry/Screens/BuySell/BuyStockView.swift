...
struct BuyStockView: View {
    ...
    var body: some View {
        VStack {
            ...
            Button {
                let result = portfolioRep.buy(ticker: stockItem.ticker, price: price.preMarket, quantity: Double(getUnitsAmount) ?? 0.0)
                if result {
                    presMode.wrappedValue.dismiss()
                }
            } label: {
...
#Preview {
    BuyStockView(stockItem: StockItem(ticker: "AAPL", name: "Apple Inc.", market: "Stocks", marketCap: 3_000_000_000_000, branding: StockItem.BrandingStock(logoUrl: "", iconUrl: "https://substackcdn.com/image/fetch/f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F8ed3d547-94ff-48e1-9f20-8c14a7030a02_2000x2000.jpeg")))
